...
<?php
    if (isset($_SESSION['status']) == 'Succes') {
        ...
        <div class="alert alert-dark alert-dismissible fade show" role="alert">
            <strong>You Have Create Your Menu Sucessfully!</strong>
    ...
    if (isset($_SESSION['update']) == 'Sucess') {
        ...
        <div class="alert alert-primary alert-dismissible fade show" role="alert">
            <strong>You Have Update Your Menu Sucessfully!</strong>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>'
    ...
    if (isset($_SESSION['delete']) == 'Sucess') {
        ...
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <strong>You Have Delete Your Menu Sucessfully!</strong>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>'
...
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#f09433', endColorstr='#bc1888',GradientType=1 );"class="text-white">Webiste Menus</h2>
    <table id="example" class="display" style="width:100%">
        ...
        <tbody>
            <?php
            $menu = mysqli_query($conn, "SELECT * FROM `websitemenu`");
            $sno = 0;
            while ($menus = mysqli_fetch_array($menu)) {
                $sno = $sno + 1;
                // print_r($menus);
                ?>
                ...
                <tr>
                   ...
                   <td>
                      <?php echo $menus['menu_url']; ?>
                    </td>
                    <td>
                        <a class='btn btn-sm btn-primary mb-3'
                            href="../websitemenu/edit_menu.php?id=<?php echo urlencode(openssl_encrypt($menus['id'], 'AES-128-ECB', 'your_secret_key')); ?>">Edit</a>

                        <a class='btn btn-sm btn-primary' href="../websitemenu/delete_menu.php?id=<?php echo urlencode(openssl_encrypt($menus['id'], 'AES-128-ECB', 'your_secret_key')); ?>"
                            class='del_btn'>Delete</a>
                    </td>
                    <!-- <td>
                        <?php
                        if ($row['status'] == 1 ) {
                            echo '<a class="btn btn-sm btn-success" href="status.php?id=' . $menus['id'] . '&status=1">Active</a>';
                        } else {
                            echo '<a class="btn btn-sm btn-danger" href="status.php?id=' . $menus['id'] . '&status=0">Inactive</a>';
                        }
                        ?>
                    </td> -->
                </tr>
